...
public class FastDeserializerGenerator<T> extends FastDeserializerGeneratorBase<T> {
    ...
    private void processComplexType(JVar fieldSchemaVar, String name, Schema schema, Schema readerFieldSchema,
            JBlock methodBody, FieldAction action, BiConsumer<JBlock, JExpression> putExpressionInRecord) {
        switch (schema.getType()) {
        case RECORD:
            processRecord(fieldSchemaVar, schema.getName(), schema, readerFieldSchema, methodBody, action, putExpressionInRecord);
            break;
        case ARRAY:
            processArray(fieldSchemaVar, name, schema, readerFieldSchema, methodBody, action, putExpressionInRecord);
            break;
        case MAP:
            processMap(fieldSchemaVar, name, schema, readerFieldSchema, methodBody, action, putExpressionInRecord);
            break;
        case UNION:
            processUnion(fieldSchemaVar, name, schema, readerFieldSchema, methodBody, action, putExpressionInRecord);
            break;
        default:
    ...
    private void processSimpletype(Schema schema, JBlock methodBody, FieldAction action,
            BiConsumer<JBlock, JExpression> putExpressionInRecord) {
        switch (schema.getType()) {
        case ENUM:
            processEnum(schema, methodBody, action, putExpressionInRecord);
            break;
        case FIXED:
            processFixed(schema, methodBody, action, putExpressionInRecord);
            break;
        default:
            processPrimitive(schema, methodBody, action, putExpressionInRecord);
        }
    }
    ...
    private void processRecord(JVar recordSchemaVar, String recordName, final Schema recordWriterSchema,
            ...
            BiConsumer<JBlock, JExpression> putValueInContainer) {
        ...
        if (methodAlreadyDefined(recordWriterSchema, recordAction.getShouldRead())) {
            ...
            if (recordAction.getShouldRead()) {
                putValueInContainer.accept(parentBody, readingExpression);
            } else {
        ...
        if (recordAction.getShouldRead()) {
            putValueInContainer.accept(parentBody, JExpr.invoke(method).arg(JExpr.direct(DECODER)));
        } else {
        ...
        for (int i = 0; i < unionSchema.getTypes().size(); i++) {
            ...
            if (SchemaAssistant.isComplexType(optionSchema)) {
                ...
                processComplexType(optionSchemaVar, optionName, optionSchema, readerOptionSchema, thenBlock,
                        unionAction, putValueInContainer);
            } else {
                processSimpletype(optionSchema, thenBlock, unionAction, putValueInContainer);
            }
        }
    }
    ...
    private void processArray(JVar arraySchemaVar, final String name, final Schema arraySchema,
        ...
        if (action.getShouldRead()) {
            putValueInContainer.accept(parentBody, arrayVar);
        }
    }

    private void processMap(JVar mapSchemaVar, final String name, final Schema mapSchema, final Schema readerMapSchema,
            JBlock parentBody, FieldAction action, BiConsumer<JBlock, JExpression> putValueInContainer) {
        ...
        if (action.getShouldRead()) {
            putValueInContainer.accept(parentBody, mapVar);
        }
    }
    ...
    private void processFixed(final Schema schema, JBlock body, FieldAction action,
        ...
        if (action.getShouldRead()) {
            ...
            putValueInContainer.accept(body, createFixed.arg(fixedBuffer));
        } else {
    ...
    private void processEnum(final Schema schema, final JBlock body, FieldAction action,
        ...
        if (action.getShouldRead()) {
            ...
            putValueInContainer.accept(body, newEnum);
        } else {
    ...
    private void processPrimitive(final Schema schema, JBlock body, FieldAction action,
        ...
        if (action.getShouldRead()) {
            ...
            putValueInContainer.accept(body, primitiveValueExpression);
        } else {
